...
//        processRequest(request, response);
        ...
        q.addQuestion(size+1+"", question, subject);
        a.addAnswer(answer, size+1);
        c.addCorrectAns(correctAns, size+1);
        ...
        for (Question question1 : dataQ) {
            ArrayList<Answer> l = a.getAllAnswerByQuesID(question1.getId());
            for (Answer answer1 : l) {
                dataA.add(answer1);
            }
        }
        for (Question question1 : dataQ) {
            ArrayList<CorrectAns> l = c.getAllCorrectAnsByQuesID(question1.getId());
